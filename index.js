...
function convertPdfToPng(pdfPath, outPrefix) {
 ...
 if (!fs.existsSync(pngPath)) return reject(new Error('PNG conversion
failed'));
 try {
...
async function main() {
 ...
 const SHEET_NAMES = (process.env.SHEET_NAMES || '').split(',').map(s =>
s.trim()).filter(Boolean);
 const START_COL = process.env.START_COL || 'F';
 const END_COL = process.env.END_COL || 'AD';
 const MAX_ROWS_PER_FILE = Number(process.env.MAX_ROWS_PER_FILE
|| '40');
 const TELEGRAM_BOT_TOKEN = process.env.TELEGRAM_BOT_TOKEN;
 const TELEGRAM_CHAT_ID = process.env.TELEGRAM_CHAT_ID;
 if (!serviceAccountJson || !SPREADSHEET_ID || !TELEGRAM_BOT_TOKEN ||
!TELEGRAM_CHAT_ID) {
...
SPREADSHEET_ID });
 ...
 console.log(`️ Loaded metadata for ${allSheets.length} sheets`);
 ...
 const rangeParam =
`${sheetName}!${START_COL}${chunk.startRow}:${END_COL}${chunk.endRow
}`;
 const exportUrl =
 `https://docs.google.com/spreadsheets/d/${SPREADSHEET_ID}/export?for
mat=pdf` +
 `&portrait=false&size=A4&fitw=true&sheetnames=false&printtitle=false&p
agenumbers=false` +
 `&gridlines=false&fzr=false&gid=${gid}&range=${encodeURIComponent(r
angeParam)}`;
 const pdfResp = await fetchPdfWithRetry(exportUrl, {
 Authorization: `Bearer ${accessToken}`
 });
 const pdfPath = path.join(tmpDir, `${sheetName}_${chunk.startRow}-
${chunk.endRow}.pdf`);
 ...
 albumImages.forEach(img => form.append(img.fileName,
...
,
 ...
 console.log('️ All sheets processed successfully');
 } catch (err) {
